...
Suppose you are part of a scrum team developing data warehouse for Bloomberg to analyze FX deals. One of customer stories is to import deals details from files into DB. Knowing that each 5 seconds around 100000 deal took place and stored in a file shared via a shared folder, the requested performance is to be able to import the file in less than 5 seconds.
...
????????
-"Validate row structure" means ? missing columns? correct ISO code?
...
1. ensure we are using single transaction()
1. used AUTO INCREMENT for Deals table. 60 sec to insert 100K, later replaced with AtomicInteger & generated myself. 35 sec to insert 100K
3. enabled batch inserts, 35 sec to insert 100K
4. Increased batch_size value to 100 and got 27 seconds to insert 100K
5. tried multithreading, but depends on how many cores the server machine has. also, JDBC driver insert inserts one by one!
6. em.flush(); & em.clear(); after batch_size(50) persist()s
7. native multi-insert query (insert into deal values (x,x,x),(x,x,x).......(x,x,x) Finally worked! 3 SECONDS!
____________________________________________
Pending:
more Exceptions,
count per currency code DONE
curr code storing as null,
testcases TDD
